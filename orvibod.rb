...
class Orvibo
	...
	def initialize
		...
		@udp.bind(0, 10000) # Socket::INADDR_ANY
	end

	def ctl(mac, on)
		if socket = sockets[mac]
			action = CTL_PREAMBLE + socket.mac + TWENTIES + (on ? CTL_ON : CTL_OFF)
			udp.send(action, 0, socket.ip, 10000)
	...
	def listen
		loop do
			...
			when ONOFF
				...
				@logger.warn "#{ip} UNHANDLED: " + packet.unpack('C*').map{|c| c.to_s(16)}.join(' ')
				next
			end
			...
			if Time.now - orvibo.subscribed_at > 120
				# send subscription
				subscribe = FBK_PREAMBLE + orvibo.mac + TWENTIES + orvibo.mac.reverse + TWENTIES
				udp.send subscribe, 0, orvibo.ip, 10000
...
class WebApp < Roda
	...
	route do |r|
		...
		r.root do
			$orvibod.sockets.map{|k, v| "#{k} is #{v.on ? 'on' : 'off'}"  }.join("<br>")
		end
